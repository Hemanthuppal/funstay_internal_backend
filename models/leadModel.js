  ...
  createLead: (data, callback) => {
    const query = `
      INSERT INTO addleads (
        ...
        another_name, another_email, another_phone_number, corporate_id, description, assignedSalesId,
    ...
    db.query(query, [...data], (err, result) => {
      ...
      // Assuming the managerid is in the appropriate position in the data array,
      // extract it for the notification (adjust the index as necessary).
      const managerid = data[15]; // Example: index 15 holds managerid
      ...
      if (managerid) {
        ...
        db.query(insertNotificationQuery, [managerid, notificationMessage], (notifErr, notifResult) => {
          if (notifErr) {
            ...
            // Continue without interrupting the callback.
          }
